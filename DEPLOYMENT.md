...
| Mục | Nội dung |
|-----|----------|
| Public URL | https://chat-production-32c8.up.railway.app |
| Platform | Railway |
...
| `REDIS_URL` | ✅ | Redis service `Redis-ExXy` trên Railway |
| `BUCKET_CAPACITY` | ✅ | 10 |
...
Dịch vụ đã kiểm tra thành công trên public URL:

```
GET /healthz → 200, status=ok
GET /readyz → 200, status=ready, redis=true
POST /chat không token → 401
POST /chat có token → 200, trả về reply và usage
Rate limit → các request vượt ngưỡng trả 429
```
...
Không sử dụng phương án dự phòng; service đã deploy thành công trên Railway.
